...
// bands, which remain the EEA hourly bands documented in METRIC_DEFS.
const US_EPA_PARTICLE_AQI_BREAKPOINTS = {
  pm25: [
...
const TRANSLATIONS = {
  en: {
    ...
    editor: {
      ...
      outdoor_pm4_entity: 'Outdoor PM4', outdoor_nox_entity: 'Outdoor NOx',
      outdoor_no2_entity: 'Outdoor NO₂', outdoor_o3_entity: 'Outdoor O₃',
      outdoor_so2_entity: 'Outdoor SO₂', outdoor_pressure_entity: 'Outdoor Pressure',
      ...
      compact_charts: 'Compact chart height',
...
class AirQualityCard extends HTMLElement {
  ...
  constructor() {
    ...
    this._history = { co2: [], pm25: [], pm1: [], pm10: [], pm03: [], pm4: [], no2: [], o3: [], so2: [], hcho: [], tvoc: [], nox: [], co: [], radon: [], radon_longterm: [], humidity: [], temperature: [], pressure: [], outdoor_co2: [], outdoor_pm25: [], outdoor_pm1: [], outdoor_pm10: [], outdoor_pm03: [], outdoor_pm4: [], outdoor_no2: [], outdoor_o3: [], outdoor_so2: [], outdoor_hcho: [], outdoor_tvoc: [], outdoor_nox: [], outdoor_co: [], outdoor_humidity: [], outdoor_temperature: [], outdoor_pressure: [] };
  ...
  setConfig(config) {
    ...
    const outdoorEntityKeys = [
      ...
      'outdoor_pm10_entity', 'outdoor_pm03_entity', 'outdoor_pm4_entity',
      'outdoor_no2_entity', 'outdoor_o3_entity', 'outdoor_so2_entity',
      'outdoor_hcho_entity', 'outdoor_tvoc_entity', 'outdoor_nox_entity', 'outdoor_co_entity',
    ...
    if (this._outdoorOnly) {
      const promotionMap = {
        ...
        outdoor_pm4_entity: 'pm4_entity',
        outdoor_no2_entity: 'no2_entity',
        outdoor_o3_entity: 'o3_entity',
        outdoor_so2_entity: 'so2_entity',
  ...
  _primaryEntityKey(metric) {
    return `${metric}_entity`;
  }

  _outdoorEntityKey(metric) {
    return `outdoor_${metric}_entity`;
  }

  _metricEntity(metric) {
    return this._config?.[this._primaryEntityKey(metric)] || this._config?.[this._outdoorEntityKey(metric)] || null;
  }

  _hasMetric(metric) {
    return !!this._metricEntity(metric);
  }

  _hasPrimaryMetric(metric) {
    return !!this._config?.[this._primaryEntityKey(metric)];
  }
  ...
  _metricValue(metric) {
    ...
    return entityId ? this._getNumericState(entityId) : null;
  }
  ...
  getCardSize() {
    ...
    if (this._hasMetric('co')) size += 1;
    if (this._config.radon_entity || this._config.radon_longterm_entity) size += 1;
    if (this._hasMetric('co2')) size += 1;
    if (this._hasMetric('pm25')) size += 1;
    if (this._hasMetric('pm10')) size += 1;
    if (this._hasMetric('pm1')) size += 1;
    if (this._hasMetric('pm03')) size += 1;
    if (this._hasMetric('no2')) size += 1;
    if (this._hasMetric('o3')) size += 1;
    if (this._hasMetric('so2')) size += 1;
    if (this._hasMetric('hcho')) size += 1;
    if (this._hasMetric('tvoc')) size += 1;
    if (this._hasMetric('pm4')) size += 1;
    if (this._hasMetric('nox')) size += 1;
    if (this._hasMetric('pressure')) size += 1;
    if (this._hasMetric('humidity')) size += 1;
    if (this._hasMetric('temperature')) size += 1;
    return size;
  }
  ...
  async _loadHistory() {
    ...
    try {
      ...
      // Outdoor sensors
      const outdoorSensors = ['co2', 'pm25', 'pm1', 'pm10', 'pm03', 'pm4', 'no2', 'o3', 'so2', 'hcho', 'tvoc', 'nox', 'co', 'humidity', 'temperature', 'pressure'];
  ...
  _getPM25Color(value, entityId) { return this._getMetricColor('pm25', value, entityId); }
  ...
  _getPM10Color(value, entityId) { return this._getMetricColor('pm10', value, entityId); }
  _getPM03Color(value) { return this._getMetricColor('pm03', value); }
  _getCOColor(value, entityId)   { return this._getMetricColor('co', value, entityId); }
  ...
  // WAQI reports its pollutant entities as US EPA individual AQI. A raw
  // indoor PM concentration is therefore not on the same graph scale. For
  // the two particle metrics with a shared 24-hour AQI window, render a
  // clearly-labelled indoor AQI *equivalent* instead. This is deliberately
  // narrower than "convert every pollutant": NO₂, O₃, SO₂, and CO have other
  // AQI averaging rules and need a dedicated rolling-average implementation.
  _usesAqiComparison(metric) {
    if (!['pm25', 'pm10'].includes(metric)) return false;
    const primaryEntityId = this._config[`${metric}_entity`];
    const outdoorEntityId = this._config[`outdoor_${metric}_entity`];
    return Boolean(primaryEntityId && outdoorEntityId) &&
      !this._isAmbientAqiEntity(metric, primaryEntityId) &&
      this._isAmbientAqiEntity(metric, outdoorEntityId) &&
      this._toAmbientUgM3(metric, 1, primaryEntityId) !== null;
  }

  // Converts a physical PM reading to a US EPA AQI-equivalent. EPA AQI is
  // formally calculated from its specified averaging period; this card draws
  // the value at each history sample, so it is expressly labelled equivalent
  // rather than presented as an official AQI/NowCast.
  _toUsEpaParticleAqiEquivalent(metric, value, entityId) {
    const breakpoints = US_EPA_PARTICLE_AQI_BREAKPOINTS[metric];
    if (!breakpoints) return null;
    if (this._isAmbientAqiEntity(metric, entityId)) return value;
    const ugM3 = this._toAmbientUgM3(metric, value, entityId);
    if (!Number.isFinite(ugM3)) return null;
    // EPA specifies truncation to one decimal for PM2.5 and to an integer for
    // PM10 before applying the linear AQI interpolation equation.
    const concentration = metric === 'pm25'
      ? Math.floor(ugM3 * 10) / 10
      : Math.floor(ugM3);
    const [lowC, highC, lowAqi, highAqi] = breakpoints.find((entry) => concentration <= entry[1]) || breakpoints[breakpoints.length - 1];
    const aqi = ((highAqi - lowAqi) / (highC - lowC)) * (concentration - lowC) + lowAqi;
  ...
  _getAqiColor(value) {
    ...
    return Number.isFinite(aqi)
      ? this._tieredValue(aqi, WAQI_AQI_THRESHOLDS, SCALE_AIRQUALITY)
      : null;
  }
  ...
  _getHCHOUnit() {
    ...
    const entityId = this._metricEntity('hcho');
    const uom = String(this._hass?.states[entityId]?.attributes?.unit_of_measurement || '').toLowerCase();
  ...
  _isVOCIndex() {
    ...
    const entityId = this._metricEntity('tvoc');
  ...
  _isNOxIndex() {
    ...
    const entityId = this._metricEntity('nox');
    if (this._hass && entityId) {
      const uom = this._hass.states[entityId]?.attributes?.unit_of_measurement;
  ...
  _getQualityColor(quality) {
    // WAQI's overall station entity is numeric AQI. Apply the same AQI
    // breakpoints used for detected WAQI pollutant entities so the header
    // leaf, number, and badge tint communicate one coherent status.
    const aqiColor = this._getAqiColor(quality);
    if (aqiColor) return aqiColor;
    const colors = {
      'good': '#4caf50',
      'excellent': '#4caf50',
      'moderate': '#8bc34a',
  ...
  _getAbnormalMetrics() {
    ...
    for (const metric of this._getMetricOrder()) {
      ...
      if (metric === 'radon') {
        ...
        value = parseFloat(this._getState(entityId));
  ...
  _initialRender() {
    ...
    const showCO = this._hasMetric('co');
    const showRadon = !!(this._config.radon_entity || this._config.radon_longterm_entity);
    const showCO2 = this._hasMetric('co2');
    const showPM25 = this._hasMetric('pm25');
    const showPM10 = this._hasMetric('pm10');
    const showPM1 = this._hasMetric('pm1');
    const showPM03 = this._hasMetric('pm03');
    const showNO2 = this._hasMetric('no2');
    const showO3 = this._hasMetric('o3');
    const showSO2 = this._hasMetric('so2');
    const showHCHO = this._hasMetric('hcho');
    const showTVOC = this._hasMetric('tvoc');
    const showPM4 = this._hasMetric('pm4');
    const showNOx = this._hasMetric('nox');
    ...
    this.shadowRoot.innerHTML = `
      ...
      <ha-card>
        <div class="card${this._config.compact_charts ? ' compact-charts' : ''}">
          ...
          <div class="graphs">
            ...
            ${showCO2 ? `
            <div class="graph-container" id="co2-graph-container" data-entity="${this._metricEntity('co2')}">
              <div class="graph-header">
                <span class="graph-label">CO₂</span>
            ...
            ${showPM1 ? `
            <div class="graph-container" id="pm1-graph-container" data-entity="${this._metricEntity('pm1')}">
              <div class="graph-header">
            ...
            ${showPM03 ? `
            <div class="graph-container" id="pm03-graph-container" data-entity="${this._metricEntity('pm03')}">
              <div class="graph-header">
            ...
            ${showHCHO ? `
            <div class="graph-container" id="hcho-graph-container" data-entity="${this._metricEntity('hcho')}">
              <div class="graph-header">
            ...
            ${showTVOC ? `
            <div class="graph-container" id="tvoc-graph-container" data-entity="${this._metricEntity('tvoc')}">
              <div class="graph-header">
            ...
            ${showPM4 ? `
            <div class="graph-container" id="pm4-graph-container" data-entity="${this._metricEntity('pm4')}">
              <div class="graph-header">
            ...
            ${showNOx ? `
            <div class="graph-container" id="nox-graph-container" data-entity="${this._metricEntity('nox')}">
              <div class="graph-header">
            ...
            ${showNO2 ? `
            <div class="graph-container" id="no2-graph-container" data-entity="${this._metricEntity('no2')}">
              <div class="graph-header">
            ...
            ${showO3 ? `
            <div class="graph-container" id="o3-graph-container" data-entity="${this._metricEntity('o3')}">
              <div class="graph-header">
            ...
            ${showSO2 ? `
            <div class="graph-container" id="so2-graph-container" data-entity="${this._metricEntity('so2')}">
              <div class="graph-header">
            ...
            ${showHumidity ? `
            <div class="graph-container" id="humidity-graph-container" data-entity="${this._metricEntity('humidity')}">
              <div class="graph-header">
            ...
            ${showTemp ? `
            <div class="graph-container" id="temperature-graph-container" data-entity="${this._metricEntity('temperature')}">
              <div class="graph-header">
            ...
            ${showPressure ? `
            <div class="graph-container" id="pressure-graph-container" data-entity="${this._metricEntity('pressure')}">
              <div class="graph-header">
  ...
  _updateStates() {
    ...
    const o3 = this._metricValue('o3');
    const so2 = this._metricValue('so2');
    const hcho = this._metricValue('hcho');
    const tvoc = this._metricValue('tvoc');
    const pm4 = this._metricValue('pm4');
    const nox = this._metricValue('nox');
    const humidity = this._metricValue('humidity');
    const temp = this._metricValue('temperature');
    const pressure = this._metricValue('pressure');
    ...
    // Update PM2.5
    if (pm25 !== null) {
      const pm25AqiComparison = this._usesAqiComparison('pm25');
      const pm25AqiValue = pm25AqiComparison
        ? this._toUsEpaParticleAqiEquivalent('pm25', pm25, this._metricEntity('pm25'))
        : null;
      const pm25Color = pm25AqiComparison ? this._getAqiColor(pm25AqiValue) : this._getPM25Color(pm25);
      const pm25ValueEl = this.shadowRoot.getElementById('pm25-value');
      if (pm25ValueEl) {
        const pm25Unit = pm25AqiComparison ? 'AQI eq.' : this._getPollutantUnit('pm25');
    ...
    // Update PM10
    if (pm10 !== null) {
      const pm10AqiComparison = this._usesAqiComparison('pm10');
      const pm10AqiValue = pm10AqiComparison
        ? this._toUsEpaParticleAqiEquivalent('pm10', pm10, this._metricEntity('pm10'))
        : null;
      const pm10Color = pm10AqiComparison ? this._getAqiColor(pm10AqiValue) : this._getPM10Color(pm10);
      const pm10ValueEl = this.shadowRoot.getElementById('pm10-value');
      if (pm10ValueEl) {
        const pm10Unit = pm10AqiComparison ? 'AQI eq.' : this._getPollutantUnit('pm10');
    ...
    if (pm4 !== null) {
      ...
      if (pm4ValueEl) {
        pm4ValueEl.innerHTML = `${pm4.toFixed(1)} <span class="unit">μg/m³</span><span class="status" id="pm4-status"></span>${outdoorSuffix('outdoor_pm4_entity', pm4, 'μg/m³')}`;
  ...
  _getPressureUnit() {
    const entityId = this._metricEntity('pressure');
    const uom = this._hass?.states[entityId]?.attributes?.unit_of_measurement;
    return uom || 'hPa';
  }
  ...
  _renderGraphs() {
    ...
    this._renderMetricGraph('tvoc', this._getTVOCColor.bind(this), 0, tvocMax, tvocUnit);
    this._renderMetricGraph('pm4', this._getPM4Color.bind(this), 0, 75, 'μg/m³');
    const noxMax = this._isNOxIndex() ? 500 : 300;
    this._renderMetricGraph('nox', this._getNOxColor.bind(this), 0, noxMax, this._getNOxUnit());
    this._renderMetricGraph('humidity', this._getHumidityColor.bind(this), 0, 100, '%');
    const tempUnit = this._getTempUnit();
    const tempMin = this._isCelsius() ? 10 : 50;
    const tempMax = this._isCelsius() ? 32 : 90;
    this._renderMetricGraph('temperature', this._getTempColor.bind(this), tempMin, tempMax, tempUnit);
    this._renderMetricGraph('pressure', this._getPressureColor.bind(this), 980, 1045, this._getPressureUnit());

    this._setupGraphInteractions();
  }

  _renderMetricGraph(metric, colorFn, minVal, maxVal, unit, formatValue) {
    const primaryData = this._history[metric] || [];
    ...
    const primaryEntityId = this._config[`${metric}_entity`];
    const outdoorEntityId = this._config[`outdoor_${metric}_entity`];
    const useAqiComparison = this._usesAqiComparison(metric);
    const canOverlayOutdoor = useAqiComparison || !hasPrimary || !hasOutdoor || this._canOverlayAmbientMetric(
      metric,
      primaryEntityId,
      outdoorEntityId
    );
    ...
    const graphPrimaryData = useAqiComparison
      ? primaryData.map(point => ({
          ...point,
          value: this._toUsEpaParticleAqiEquivalent(metric, point.value, primaryEntityId)
        })).filter(point => point.value !== null)
      : convertForDisplay(primaryData, primaryEntityId);
    const graphOutdoorData = useAqiComparison
      ? outdoorData
      : convertForDisplay(outdoorData, outdoorEntityId);
    ...
    const graphColorFn = useAqiComparison
      ? (value) => this._getAqiColor(value)
      : (value) => colorFn(value, displayEntityId);
    // Incompatible sources (for example indoor PM2.5 in µg/m³ and WAQI
    // PM2.5 AQI) must not share a plotted Y-axis. Keep the exterior history
    // available to the tooltip, where its own unit makes the distinction
    // explicit, without drawing a scientifically false dashed line.
    const tooltipOnlyOutdoor = hasPrimary && hasOutdoor && !canOverlayOutdoor;
    const tooltipOptions = tooltipOnlyOutdoor ? {
      tooltipOutdoorData: outdoorData,
      tooltipOutdoorUnit: this._isAmbientAqiEntity(metric, outdoorEntityId)
        ? 'AQI'
        : this._getEntityUnit(outdoorEntityId, unit),
      tooltipOutdoorColorFn: (value) => colorFn(value, outdoorEntityId)
    } : {};

    const graphUnit = useAqiComparison ? 'AQI' : unit;
    const graphMin = useAqiComparison ? 0 : minVal;
    const graphMax = useAqiComparison ? 240 : maxVal;
    const primaryLabel = useAqiComparison ? 'Indoor AQI equivalent' : 'Indoor';
    const outdoorLabel = useAqiComparison ? 'Outdoor AQI' : 'Outdoor';
    ...
    if (hasPrimary && graphPrimaryData.length) {
      this._renderGraph(
        ...
        outdoorLabel,
        formatValue,
        hasOutdoor ? { primaryLabel, ...tooltipOptions } : {}
      );
      ...
      this._renderGraph(
        ...
        outdoorLabel,
        formatValue,
        { primaryLabel: 'Outdoor', primaryLineStyle: 'dashed' }
      );
    }
  }
  ...
  _renderGraph(graphId, data, colorFn, minVal, maxVal, unit, outdoorData, outdoorLabel, formatValue, options = {}) {
    ...
    // A tooltip-only exterior series is intentionally not part of the graph
    // scale or SVG. It is used when the two data sources have incompatible
    // units/scales but the user should still be able to inspect both values
    // at the same point in time.
    const tooltipOutdoorData = options.tooltipOutdoorData;
    const tooltipOutdoorPoints = tooltipOutdoorData && tooltipOutdoorData.length
      ? tooltipOutdoorData.map(d => ({
          x: this._computeGraphX(d.time, width, padding),
          value: d.value,
          time: d.time,
          color: (options.tooltipOutdoorColorFn || colorFn)(d.value)
        }))
      : outdoorPoints;

    this._graphData[graphId] = {
      points,
      outdoorPoints: tooltipOutdoorPoints,
      unit,
      colorFn,
      formatValue,
      primaryLabel: options.primaryLabel || '',
      outdoorLabel: outdoorLabel || 'Outdoor',
      outdoorUnit: options.tooltipOutdoorUnit || unit,
      outdoorFormatValue: options.tooltipOutdoorFormatValue || formatValue
    };
    ...
    const primaryLineAttrs = options.primaryLineStyle === 'dashed' ? ' stroke-dasharray="4 3" opacity="0.75"' : '';
    ...
    if (outdoorPoints.length >= 2) {
      ...
      <path d="${linePath}" stroke="url(#${gradientId})" class="graph-line" fill="none"${primaryLineAttrs} />
  ...
  _updateCursor(graphId, event) {
    ...
    const defaultFormatVal = (val, activeUnit) => {
      if (activeUnit === 'ppm' || activeUnit === 'ppb' || activeUnit === 'AQI' || activeUnit === 'p/0.1L' || activeUnit === 'Bq/m³') return Math.round(val);
      if (activeUnit === '%' || activeUnit === '°F' || activeUnit === '°C') return Math.round(val);
      if (activeUnit === 'pCi/L') return val.toFixed(1);
      return val.toFixed(1);
    };
    const formatVal = data.formatValue || ((val) => defaultFormatVal(val, data.unit));
    const withUnit = (val, activeUnit = data.unit, formatter = formatVal) =>
      `${formatter(val)}${activeUnit ? ` ${activeUnit}` : ''}`;

    if (valueEl) {
      const displayValue = withUnit(closest.value);
      valueEl.textContent = data.primaryLabel ? `${data.primaryLabel}: ${displayValue}` : displayValue;
    ...
    if (outdoorEl) {
      if (data.outdoorPoints && data.outdoorPoints.length) {
        ...
        const outdoorFormat = data.outdoorFormatValue || ((val) => defaultFormatVal(val, data.outdoorUnit));
        outdoorEl.textContent = `${data.outdoorLabel}: ${withUnit(closestOutdoor.value, data.outdoorUnit, outdoorFormat)}`;
  ...
  class AirQualityCardEditorBase extends LitElement {
    ...
    _computeLabel = (schema) => {
      ...
      const localFallbacks = {
        ...
        compact_charts: 'Compact chart height',
        outdoor_pm4_entity: 'Outdoor PM4',
      ...
      return [
        ...
        },
        {
          ...
          schema: [
            ...
            },
            {
              type: 'grid',
              schema: [
                { name: 'outdoor_pm4_entity', selector: { entity: { domain: 'sensor' } } },
        ...
        },
        {
          ...
          schema: [
            ...
            { name: 'compact_charts', selector: { boolean: {} } },
